# Ported over from: https://github.com/ProgBelarus/BatchMultivalidConformal/blob/main/src/utils/MultivalidPlotting.py
import numpy as np
...
def plot_all_group_coverage(model1, model2, tau, x_test, y_test, num_groups, group_fn, marginal_conformal_coverage, group_conformal_coverage, multivalid=False, num_grid=100, bar_width=0.25):
    ...
    plt.bar(br3, group_coverage1, color = 'c', width = bar_width, edgecolor = 'gray', label = 'BatchGCP', linewidth = 0.5)
    plt.bar(br4, group_coverage2, color = 'g', width = bar_width, edgecolor = 'gray', label = 'BatchMVP', linewidth = 0.5)
    plt.axhline(y=tau, c='r', linestyle='--', linewidth=2)
...
def plot_all_pred_set_sizes(model1, model2, tau, x_test, y_test, num_groups, group_fn, mult_width, marginal_conformal_size, group_conformal_size, num_grid=100, multivalid=False, bar_width = 0.25):
    ...
    plt.bar(br3, group_pred_set_size1, color = 'c', width = bar_width, edgecolor = 'gray', label = 'BatchGCP', linewidth = 0.5)
    plt.bar(br4, group_pred_set_size2, color = 'g', width = bar_width, edgecolor = 'gray', label = 'BatchMVP', linewidth = 0.5)
    plt.xlabel('Group Number')
    plt.xticks(range(num_groups), range(1, num_groups+1))
    plt.ylabel('Mean prediction set size')
